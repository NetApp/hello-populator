...
import (
	...
	helloclientset "github.com/kubernetes-csi/hello-populator/client/clientset/versioned"
	hellofactory "github.com/kubernetes-csi/hello-populator/client/informers/externalversions"
	helloinformers "github.com/kubernetes-csi/hello-populator/client/informers/externalversions/hello/v1alpha1"
	hellolisters "github.com/kubernetes-csi/hello-populator/client/listers/hello/v1alpha1"
)
...
func runController(masterURL, kubeconfig, imageName string) {
	...
	controller := newController(kubeClient, imageName,
...
func (c *controller) syncPvc(ctx context.Context, key, namespace, pvcName string) error {
	...
	if "" == pvc.Spec.VolumeName {
		...
		if finalizers, needUpdate := ensureFinalizer(pvc, pvcFinalizer, true); needUpdate {
			...
			_, err = c.kubeclientset.CoreV1().PersistentVolumeClaims(namespace).Update(ctx, pvc, metav1.UpdateOptions{})
			if nil != err {
				return err
			}
		}
		...
		if 0 == len(podList) {
			_, err = c.kubeclientset.CoreV1().Pods(workingNamespace).Create(ctx, makePopulatePod(labelValue, pvc, c.imageName, hello), metav1.CreateOptions{})
			if nil != err {
				return err
			}
			...
			if 0 == len(pvcList) {
				_, err = c.kubeclientset.CoreV1().PersistentVolumeClaims(workingNamespace).Create(ctx, makeRestorePvc(labelValue, pvc), metav1.CreateOptions{})
				if nil != err {
					return err
				}
			}
		...
		if corev1.PodSucceeded != pod.Status.Phase {
			if corev1.PodFailed == pod.Status.Phase {
				...
				err = c.kubeclientset.CoreV1().Pods(workingNamespace).Delete(ctx, pod.Name, metav1.DeleteOptions{})
				if nil != err {
					return err
				}
			}
		...
		pv, err = c.kubeclientset.CoreV1().PersistentVolumes().Get(ctx, pvcPrime.Spec.VolumeName, metav1.GetOptions{})
		if nil != err {
			if errors.IsNotFound(err) {
		...
		if claimRef.Name != pvcName || claimRef.Namespace != pvc.Namespace ||
			...
			pv.Labels[targetLabel] = labelValue
			...
			_, err = c.kubeclientset.CoreV1().PersistentVolumes().Update(ctx, pv, metav1.UpdateOptions{})
			if nil != err {
				return err
			}
	...
	if 0 < len(pvcList) {
		pvcPrime := pvcList[0]
		err = c.kubeclientset.CoreV1().PersistentVolumeClaims(workingNamespace).Delete(ctx, pvcPrime.Name, metav1.DeleteOptions{})
		if nil != err {
			return err
		}
	}
